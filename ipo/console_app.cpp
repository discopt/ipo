...
namespace ipo {
  ...
  void ConsoleApplicationBase::printUsage()
  {
    ...
    std::cerr
        << " --smallest-face              Computes, for every given point in P, an objective vector that induces\n";
    ...
    std::cerr
        << " --faces FILE                 Each line of the file is considered as a single --face argument. If it\n";
    std::cerr
        << "                              does not match the pattern, the line is silently ignored, allowing it\n";
    ...
    std::cerr
        << " --random n                   Sample n objective vectors (uniformly at random from the sphere) to be\n";
    ...
    std::cerr
        << " --directions FILE            Each line of the file is considered as a single --directions argument.\n";
  ...
  int ConsoleApplicationBase::parseArgument(const std::string& firstArgument)
  {
    ...
    if (firstArgument == "--random")
    {
      ...
      int n;
      ss >> n;
      std::cout << "stream: " << (bool(ss) ? "true" : "false") << ", good: " << (ss.good() ? "true" : "false")
          << std::endl;
      if (n <= 0)
        throw std::runtime_error("Invalid option: --random must be followed by a positive number.");
  ...
  bool ConsoleApplicationBase::processArguments()
  {
    ...
    _equations = new soplex::LPRowSetRational;
  ...
  bool ConsoleApplicationBase::optimizeObjective(const soplex::SVectorRational* objective, bool maximize)
  {
    return true;
  }

  bool ConsoleApplicationBase::generateFacets(const soplex::SVectorRational* objective)
  {
    ...
    for (std::size_t v = 0; v < n; ++v)
      cols.add(0, -soplex::infinity, vector, soplex::infinity);
