...
void run(std::shared_ptr<ipo::GurobiSolver> gurobi, std::shared_ptr<ipo::OptimizationOracle<Number>> baseOracle,
  ...
  bool outputInterior, bool outputInstanceFacets, int outputRandomFacets)
{
  ...
  if (outputInstanceFacets || outputRandomFacets)
  {
    ...
    std::size_t gurobiVar = 0;
    bool first = true;
    for (std::size_t c = 0; c < poly->space()->dimension(); ++c)
    {
      const std::string& name = poly->space()->variable(c);
      while (baseOracle->space()->variable(gurobiVar) != name)
      {
        ++gurobiVar;
        assert(gurobiVar < baseOracle->space()->dimension());
      }
      lp.addColumn(lp.minusInfinity(), lp.plusInfinity(), Number(gurobi->instanceObjective()[gurobiVar]), name);
      if (gurobi->instanceObjective()[gurobiVar])
      {
        std::cerr << (first ? "Objective: " : " + ") << gurobi->instanceObjective()[gurobiVar] << "*"
          << baseOracle->space()->variable(gurobiVar);
        first = false;
      }
    }
    std::cerr << std::endl;
...
int main(int argc, char** argv)
{
...
#endif /* IPO_DOUBLE && IPO_RATIONAL */
  {
    ...
    run<ipo::rational>(gurobi, gurobi->getOptimizationOracle<ipo::rational>(), gurobi->getSeparationOracle<ipo::rational>(),
...
#endif /* IPO_DOUBLE && IPO_RATIONAL */
  {
    ...
    run<double>(gurobi, gurobi->getOptimizationOracle<double>(), gurobi->getSeparationOracle<double>(), false,
