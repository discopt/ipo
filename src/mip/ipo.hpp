...
namespace ipo
{
  ...
  void run(std::shared_ptr<ipo::OptimizationOracle<Number>> baseOracle,
    std::shared_ptr<ipo::SeparationOracle<Number>> sepa, const double* instanceObjective, bool gmp, double timeLimit,
    int randomSeed, const std::string& projectionRegex, bool useDominant, bool useSubmissive, bool outputDimension,
    bool outputEquations, bool outputInterior, bool outputInstanceFacets, int outputRandomFacets)
  {
    ...
    if (outputInstanceFacets || outputRandomFacets)
    {
      ...
      for (std::size_t c = 0; c < poly->space()->dimension(); ++c)
      {
        ...
        while (baseOracle->space()->variable(solverVar) != name)
        {
          ++solverVar;
          assert(solverVar < baseOracle->space()->dimension());
        }
        lp.addColumn(lp.minusInfinity(), lp.plusInfinity(), Number(instanceObjective[solverVar]), name);
        if (instanceObjective[solverVar])
        {
          std::cerr << (first ? "Objective: " : " + ") << instanceObjective[solverVar] << "*"
            << baseOracle->space()->variable(solverVar);
          first = false;
        }
      }
      std::cerr << std::endl;
